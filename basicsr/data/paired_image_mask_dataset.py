...
class PairedImageMaskDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            img_mask = imfrombytes(img_bytes, float32=True, flag='grayscale')  # Assuming mask is a grayscale image
        except:
        ...
        if self.opt['phase'] == 'train':
            ...
            # padding
            img_gt, img_lq, img_mask = padding_with_mask(img_gt, img_lq, img_mask, gt_size)

            # random crop
            img_gt, img_lq, img_mask = paired_random_crop_with_mask(img_gt, img_lq, img_mask, gt_size, scale,
                                                gt_path)
            # flip, rotation
            img_gt, img_lq, img_mask = augment([img_gt, img_lq, img_mask], self.opt['use_flip'],
                                     self.opt['use_rot'])
...
def triple_paths_from_meta_info_file(folders, keys, meta_info_file, filename_tmpl):
    ...
    for gt_name in gt_names:
        ...
        mask_name = f'{filename_tmpl.format(basename)}{ext}'

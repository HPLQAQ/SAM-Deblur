...
def mask_collate_fn(batch):
    # Separate the list of dictionaries into individual lists
    img_lq_list = [item['lq'] for item in batch]
    img_gt_list = [item['gt'] for item in batch]
    img_mask_list = [item['masks'] for item in batch]
    img_lq_path_list = [item['lq_path'] for item in batch]
    img_gt_path_list = [item['gt_path'] for item in batch]

    img_lq_stacked = torch.stack(img_lq_list, dim=0)
    img_gt_stacked = torch.stack(img_gt_list, dim=0)

    return {
        'lq': img_lq_stacked,
        'gt': img_gt_stacked,
        'masks': img_mask_list,
        'lq_path': img_lq_path_list,
        'gt_path': img_gt_path_list,
    }
...
def create_train_val_dataloader(opt, logger):
    ...
    collate_fn = None
    if opt.get('grouped_masks', False):

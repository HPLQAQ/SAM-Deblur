...
class ImageRestorationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.grouped_masks = opt.get('grouped_masks', False)
    ...
    def feed_data(self, data, is_val=False):
        ...
        if self.grouped_masks:
            self.masks = [mask.to(self.device) for mask in data['masks']]
        if 'gt' in data:
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        if self.grouped_masks:
            preds = self.net_g(self.lq, self.masks)
        else:
        ...
        with torch.no_grad():
            ...
            while i < n:
                ...
                if j >= n:
                    j = n
                if self.grouped_masks:
                    pred = self.net_g(self.lq[i:j], self.masks[i:j])
                else:
